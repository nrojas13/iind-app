...
export class CursoComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(
      (params: Params) => {
        ...
        this.http.get('http://localhost:3000/cursos/' + this.curso_id)
  ...
  onCrearProfesor() {
    ...
    this.http.post("http://localhost:3000/profesores/", JSON.stringify(this.profesor), { headers: headers })
      .map(res => res.json())
      .subscribe(result1 => {
        ...
        this.http.post("http://localhost:3000/profesores/" + profesor.id + "/asignar_curso", JSON.stringify(json), { headers: headers })
        .subscribe(result2 => {
          this.showAlert();
        });
      });
