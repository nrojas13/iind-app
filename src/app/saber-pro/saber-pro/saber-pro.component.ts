...
export class SaberProComponent implements OnInit {
  ...
  ngOnInit() {
    this.http.get('https://iindinfo.herokuapp.com/saber_pros')
      .map(response => response.json())
      .subscribe(result => {
        this.saberPro = [];
        result.forEach(element => {
          this.http.get('https://iindinfo.herokuapp.com/estudiantes/' + element.estudiante_id)
  ...
  public cargarPagos(event) {
        ...
        if(fileList.length > 0) {
            ...
            this.http.post('https://iindinfo.herokuapp.com/saber_pros/cargar_archivo_pagos', formData)
                .map(res => res.json())
                .subscribe(
